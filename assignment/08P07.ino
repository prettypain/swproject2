...
#define tic     255/100   //100~200까지와 200~300까지는 대칭이기 때문에 100으로 나눔
...
void loop() {
  ...
  if ((distance == 0.0) || (distance > _DIST_MAX)) {
      ...
      analogWrite(PIN_LED, tic*(abs(distance-200)));
  }
...
//샘플링 주기가 일정하지 않은 이유
샘플 08은 milles함수를 통해서 시작한 시간지 얼마나 지났는지에 대한 시간을 측적해
이를 이전 샘플링 시간과 비교함으로써 거리를 측정하는데 걸리는 시간을 포함하지 않고 딜레이를
일정하게 줄 수 있다.
